...
BASELINE_LOG="/tmp/zerokernel_esp32_baseline.log"
KERNEL_LOG="/tmp/zerokernel_esp32_kernel.log"
BASELINE_BUILD_LOG="/tmp/zerokernel_esp32_baseline_build.log"
KERNEL_BUILD_LOG="/tmp/zerokernel_esp32_kernel_build.log"
RESTORE_BUILD_LOG="/tmp/zerokernel_esp32_restore_build.log"
...
def parse_resource(path):
    ...
    flash = re.search(r"Sketch uses (\d+) bytes \((?:\d+)%\) of program storage space. Maximum is (\d+) bytes.", text)
    if ram and flash:
        return {
            "ram": tuple(int(x) for x in ram.groups()),
            "flash": tuple(int(x) for x in flash.groups()),
        }
...
print("deadline_miss:", baseline.get("deadline_miss", 0), "->", kernel.get("deadline_miss", 0))
print("queue_drop:", baseline.get("queue_drop", 0), "->", kernel.get("queue_drop", 0))
PY
