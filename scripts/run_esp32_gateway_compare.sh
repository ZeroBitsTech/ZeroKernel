...
BASELINE_LOG="/tmp/zerokernel_esp32_gateway_baseline.log"
MODULE_LOG="/tmp/zerokernel_esp32_gateway_module.log"
BASELINE_BUILD_LOG="/tmp/zerokernel_esp32_gateway_baseline_build.log"
MODULE_BUILD_LOG="/tmp/zerokernel_esp32_gateway_module_build.log"
RESTORE_BUILD_LOG="/tmp/zerokernel_esp32_gateway_restore.log"
...
echo "Running ESP32 TelemetryGateway baseline on ${PORT}"
compile_and_upload \
  "${ROOT_DIR}/examples/TelemetryGatewayBaseline" \
  "${BASELINE_BUILD_LOG}"
capture_serial "${BASELINE_LOG}"
extract_line 'TELEMETRY_GATEWAY' "${BASELINE_LOG}"

echo "Running ESP32 TelemetryGateway ZeroKernel on ${PORT}"
compile_and_upload \
  "${ROOT_DIR}/examples/TelemetryGatewayNode" \
  "${MODULE_BUILD_LOG}"
capture_serial "${MODULE_LOG}"
extract_line 'TELEMETRY_GATEWAY' "${MODULE_LOG}"
...
base_line, base = parse_last("TELEMETRY_GATEWAY", base_path)
node_line, node = parse_last("TELEMETRY_GATEWAY", node_path)
base_res = parse_resource(base_build_path)
node_res = parse_resource(node_build_path)

print("---- telemetry gateway compare ----")
print(base_line)
print(node_line)
print("ram_bytes:", describe(base_res["ram"], node_res["ram"]))
print("flash_bytes:", describe(base_res["flash"], node_res["flash"]))
for key in ("link_up", "wifi_attempts", "reconnects", "http_ok", "http_fail", "http_rate", "mqtt_ok", "mqtt_fail", "mqtt_rate", "http_queue", "mqtt_queue"):
